...
int main(int argc, char **argv)
{
	...
	/*Multithreading-ness!!*/
	LPHANDLE phThreads = new HANDLE[argc + 1];
	LPDWORD pdwThreadIDs = new DWORD[argc + 1];
	InitializeCriticalSection(&cs);

	for (i = 1; i<argc; i++)
	{
		/*process the ith file*/
		phThreads[i] = CreateThread(NULL, 0, Run, argv[i], 0, &pdwThreadIDs[i]);
	...
	// Added for Multithreading
	// Waiting and Joining all Threads
	for (i = 1; i<argc; i++)
