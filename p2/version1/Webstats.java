...
public class Webstats implements Runnable
{
	...
	*/
	...
	/*
 	Added for running the threads for Multithreading
	*/
	public void run()
	{
		process_file(Thread.currentThread().getName());
	}
	...
	public static void main(String argv[])
	{
		...
		Webstats webstatistics = new Webstats("Webstats.java");
		
		//Doing the multithreading thing
		if(argv.length == 1){
			webstatistics.process_file(argv[0]);
		}else{
			Thread threads[] = new Thread[argv.length];
			for (int i = 0; i < threads.length; i++){
				threads[i] = new Thread(webstatistics, argv[i]);
				threads[i].start();
			}
